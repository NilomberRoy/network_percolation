...
void seed_initialization()
{
    ...
    // Prepare the node_list for preferential attachment
    
    for (int i = 0; i < m0; ++i) {
        int deg = Nodes_Neigh[i].size(); // degree of node i
        for (int d = 0; d < deg; ++d) {
            node_list.push_back(i); // Add i multiple times based on degree, number of appearence of each node in indices equal to its degree
        }
    }
}
/*In network_build() function we used a loop to build BA network
by iterative method. Per unit time, new node arrives and it chose 
m existing nodes via preferential attachment, which means the probability
of picking a node is higher wwhich have higher degree than others. */
void network_build() {
    seed_generator();
    unordered_set<int> targets; // Declare once outside for reuse

    for (int i = m0; i < N; ++i) {

        uniform_int_distribution<> distribution(0, node_list.size() - 1);

        while (targets.size() < m) {
            int candidate = node_list[distribution(gen)];

            if (candidate != i && targets.insert(candidate).second) {
                // Connect i and candidate
                Nodes_Neigh[i].push_back(candidate);
                Nodes_Neigh[candidate].push_back(i);

                NodeA.push_back(i);
                NodeB.push_back(candidate);

                // Update node_list for new degrees
                node_list.push_back(i);
                node_list.push_back(candidate);
            }
        }
        unordered_set<int>().swap(targets);
    }
...
{   
    ...
    auto start_time = steady_clock::now();  // Start time for the whole percolation process
    for (int ensemble = 0; ensemble < ensemble_count; ensemble++)
    {   
        auto ensemble_start_time = steady_clock::now();  // Start time for the current ensemble
        Nodes_Neigh.clear();
     ...
     // Reset the network and bond configurations for each ensemble
        ...
        auto ensemble_end_time = steady_clock::now();  // End time for the current ensemble
        auto ensemble_duration = duration_cast<seconds>(ensemble_end_time - ensemble_start_time);
        cout << "Ensemble " << ensemble << " took " << ensemble_duration.count() << " seconds." << endl;
	}
...
int main(int argc, char* argv[]){
    ...
    t_Nalpha="BA_t_Nalpha_M"+to_string(M)+"_N"+to_string(N)+"_m0"+to_string(m0)+"_en"+to_string(ensemble_count)+"_m"+to_string(m)+"_alpha"+to_string(alpha)+"_fnum"+to_string(filenum)+".dat";
    powder="BA_powder_M"+to_string(M)+"_N"+to_string(N)+"_m0"+to_string(m0)+"_en"+to_string(ensemble_count)+"_m"+to_string(m)+"_alpha"+to_string(alpha)+"_fnum"+to_string(filenum)+".dat";
    largestclus="BA_op_M"+to_string(M)+"_N"+to_string(N)+"_m0"+to_string(m0)+"_en"+to_string(ensemble_count)+"_m"+to_string(m)+"_alpha"+to_string(alpha)+"_fnum"+to_string(filenum)+".dat";
    ent="BA_ent_M"+to_string(M)+"_N"+to_string(N)+"_m0"+to_string(m0)+"_en"+to_string(ensemble_count)+"_m"+to_string(m)+"_alpha"+to_string(alpha)+"_fnum"+to_string(filenum)+".dat";
    specific_heat="BA_spe_M"+to_string(M)+"_N"+to_string(N)+"_m0"+to_string(m0)+"_en"+to_string(ensemble_count)+"_m"+to_string(m)+"_alpha"+to_string(alpha)+"_fnum"+to_string(filenum)+".dat";
    susceptibility="BA_sus_M"+to_string(M)+"_N"+to_string(N)+"_m0"+to_string(m0)+"_en"+to_string(ensemble_count)+"_m"+to_string(m)+"_alpha"+to_string(alpha)+"_fnum"+to_string(filenum)+".dat";
    u="BA_U_M"+to_string(M)+"_N"+to_string(N)+"_m0"+to_string(m0)+"_en"+to_string(ensemble_count)+"_m"+to_string(m)+"_alpha"+to_string(alpha)+"_fnum"+to_string(filenum)+".dat";
	percolation();
